...
from padl.transforms import ClassTransform, Identity, Base, Mode
...
class IfInMode(ClassTransform):
    ...
    :param if_: Base to apply when the mode matches.
    :param target_mode: Mode (one of 'train', 'eval', 'infer').
    :param else_: Base to apply when the mode doesn't match (defaults to identity transform).
    """

    pd_dont_dump_code = True

    def __init__(self, if_: Base, target_mode: Mode, else_: Optional[Base] = None):
    ...
    def __call__(self, args):
        ...
        if Base.pd_mode == self.target_mode:
...
class IfInfer(IfInMode):
    ...
    :param if_: Base for the "infer" mode.
    :param else_: Base otherwise (defaults to the identity transform).
    """

    def __init__(self, if_: Base, else_: Optional[Base] = None):
...
class IfEval(IfInMode):
    ...
    :param if_: Base for the "eval" mode.
    :param else_: Base otherwise (defaults to the identity transform).
    """
...
class IfTrain(IfInMode):
    ...
    :param if_: Base for the "train" mode.
    :param else_: Base otherwise (defaults to the identity transform).
    """
...
class Try(ClassTransform):
    ...
    *catch_transform*. If *transform* is completed successfully, *else_transform* is performed
    with the output of *transform*. Regardless of any error occurring on the other Base
    (or not), *finally_transform* is carried out. No change of mode can happen inside any of these
    Base.

    :param transform: Base to try.
    :param catch_transform: Base to fall back on.
    :param exceptions: Catch conditions.
    :param else_transform: Base to carry on the `else` clause of the `try` statement.
    :param finally_transform: Base to carry on on the `finally` clause of the `try` statement.
    :param pd_name: The Base's name.
    """

    def __init__(self,
                 transform: Base,
                 catch_transform: Base,
                 exceptions: Union[List, Tuple, Exception],
                 else_transform: Base = Identity(),
                 finally_transform: Base = Identity(),
                 pd_name: str = None):
        ...
        assert len(self._pd_stage) == 1, 'Try Base cannot contain transforms that have ' \
                                            'multiple components.'
