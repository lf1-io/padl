...
def test__get_call_signature_a():
    ...
    assert signature == (['x', 'y', 'z'], {}, None, None)


def test__get_call_signature_b():
    source = 'a(x, y, z=1)'
    signature = symfinder._get_call_signature(source)
    assert signature == (['x', 'y'], {'z': '1'}, None, None)


def test__get_call_assignments_a():
    source = 'def f(x, y, z=1):pass'
    args = ast.parse(source).body[0].args
    values, keywords, _, _ = symfinder._get_call_signature('f(1, 2, 3)')
    assignments = symfinder._parse_def_args(args, source).get_call_assignments(values, keywords)
...
def test__get_call_assignments_b():
    source = 'def f(x, y, z=1):pass'
    args = ast.parse(source).body[0].args
    values, keywords, _, _ = symfinder._get_call_signature('f(1, 2)')
    assignments = symfinder._parse_def_args(args, source).get_call_assignments(values, keywords)
...
def test__get_call_assignments_c():
    ...
    args = ast.parse(source).body[0].args
    values, keywords, _, _ = symfinder._get_call_signature('f(1, 2, 3, z=4)')
    assignments = symfinder._parse_def_args(args, source).get_call_assignments(values, keywords)
...
def test__get_call_assignments_e():
    ...
    args = ast.parse(source).body[0].args
    values, keywords, _, _ = symfinder._get_call_signature('f(1, 2, 3, z=4, u=7, f=8)')
    assignments = symfinder._parse_def_args(args, source).get_call_assignments(values, keywords)
